...
export class SpaceInvadersFormationSystem extends GameSystem {
  ...
  private columnShooterEntities: Int32Array = new Int32Array(0);
  private columnShooterY: Float32Array = new Float32Array(0);
  private compactShooters: Int32Array = new Int32Array(0);
  ...
  private ensureArrays(cols: number): void {
  ...
  public update(world: World<SpaceInvadersComponentRegistry>, deltaTime: number): void {
    ...
    // Check if formation is stunned by EMP blast wave
    if (formation.stunnedRemaining && formation.stunnedRemaining > 0) {
      ...
      const nextStun = Math.max(0, formation.stunnedRemaining - dtSeconds);
      world.mutateComponent(formationEntity, "Formation", f => {
        f.stunnedRemaining = nextStun;
      });
      return;
    }
    ...
    const rightLimit = formation.rightBound !== undefined && formation.rightBound > 0 ? formation.rightBound : GAME_CONFIG.worldWidth - margin;
  ...
  private fireFromFormation(world: World<SpaceInvadersComponentRegistry>, invaderEntities: ReadonlyArray<number>): void {
    const config = this.getGameConfig(world);
    const maxCols = Math.max(config.INVADER_COLS, 30);
    this.ensureArrays(maxCols);

    this.columnShooterEntities.fill(-1);
    this.columnShooterY.fill(-Infinity);
    ...
    for (let i = 0; i < len; i++) {
      ...
      const pos = world.getComponent(entity, "Transform");
      if (invader && pos && invader.col >= 0 && invader.col < maxCols) {
    ...
    for (let c = 0; c < maxCols; c++) {
      const entity = this.columnShooterEntities[c];
      if (entity !== -1) {
