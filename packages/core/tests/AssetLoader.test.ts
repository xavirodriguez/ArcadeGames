...
describe("AssetLoader validation", () => {
  ...
  it("should throw an error when loading assets and no provider is registered", async () => {
    const loaderNoProvider = new AssetLoader();
    const assets = [
      { id: "test_img", path: "test/img.png", type: "image" as const }
    ];
    await expect(loaderNoProvider.load(assets)).rejects.toThrow("AssetLoader: no provider registered");
  });
});
