...
export const ChoiceCard: React.FC<ChoiceCardProps> = ({ choice, onSelect }) => {
  ...
  return (
    <button
      ...
      className={`w-full text-left p-4 rounded-md border transition-all duration-200 group relative focus:outline-none focus:ring-2 focus:ring-cyan-400 active:scale-[0.99] ${
        isIrreversible
          ? 'bg-rose-950/20 border-rose-600/60 hover:bg-rose-900/40 hover:border-rose-500'
          : 'bg-slate-900/80 border-slate-700/80 hover:bg-slate-800 hover:border-cyan-500/80'
      }`}
    ...
    >
      <div className="flex flex-col gap-1.5">
        <div className="flex items-center justify-between gap-2">
          ...
          {isIrreversible && (
            <span
              className="inline-flex items-center gap-1.5 text-xs font-mono font-bold uppercase text-rose-400 bg-rose-950/80 border border-rose-600/80 px-2 py-0.5 rounded tracking-wide animate-pulse"
