...
> {
  ...
  /** @internal */
  private componentRegistry = new Map<string, { allowMutationDuringUpdate?: boolean }>();

  /**
   * Registers component options/metadata.
   */
  public registerComponentMetadata(type: string, options: { allowMutationDuringUpdate?: boolean }): void {
    this.componentRegistry.set(type, options);
  }
  ...
  constructor(schedule?: Schedule<TComponents, TEvents, TBlueprints>) {
    ...
    this.registerComponentMetadata("Juice", { allowMutationDuringUpdate: true });
    this.registerComponentMetadata("VisualOffset", { allowMutationDuringUpdate: true });
  }
  ...
  private checkUpdatingMutation(opName: string, type?: string): void {
    if (isDev && this.isUpdating) {
      if (type && this.componentRegistry.get(type)?.allowMutationDuringUpdate) {
        return;
      }
