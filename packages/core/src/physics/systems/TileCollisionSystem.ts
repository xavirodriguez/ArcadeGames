...
export class TileCollisionSystem<TRegistry extends ComponentRegistry = CoreComponentRegistry> extends System<TRegistry> {
  ...
  public update(world: World<TRegistry>, deltaTime: number): void {
    ...
    for (let i = 0; i < len; i++) {
      ...
      xTileLoop: for (// TODO(refactor): código duplicado detectado (bloque) con physics/systems/TileCollisionSystem.ts:162-170. Considerar extraer a función compartida. Ref: c358e011
        ...
        for (let tx = minTileX; tx <= maxTileX; tx++) {
          ...
          if (tileDef.solid) {
            if (tileDef.oneWay) {
              ...
              // TODO(refactor): código duplicado detectado (bloque) con physics/systems/TileCollisionSystem.ts:191-199. Considerar extraer a función compartida. Ref: a710136f
