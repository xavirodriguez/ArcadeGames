...
* **Resultado de verificación:** No existe `registerComponent(type: string)` en `World.ts`. El método público real es `registerComponentMetadata(type: string, options: { allowMutationDuringUpdate?: boolean })`. En el ECS de Tiny Aster, los tipos de componentes se registran dinámicamente al invocarse `addComponent`. Los tests que invocaban un hipotético `registerComponent` dependían de APIs deprecadas o inexistentes y no deben tomarse como referencia.
...
* **Implementación:** Fusionar y ejecutar el test `BroadPhaseWorstCase.benchmark.test.ts` evaluando tres escenarios críticos:
...
* **Criterio de salida (Gate):** Si ningún escenario empuja el tiempo de ejecución del broadphase por encima del ~15% del *frame budget* (aprox. 2.5ms sobre 16.6ms con 500+ entidades), **este roadmap se da por concluido**. El algoritmo actual (`BroadPhase.sweepAndPrune`) se considera definitivo y la Fase 2 queda abortada por innecesaria.

## Fase 2 — `SpatialHashGrid` (Condicional)
*Solo se ejecutará si la Fase 1 demuestra un cuello de botella real en dispositivos/escenarios objetivo.*
...
* **Criterios de Equivalencia:**
  ...
  * Los tests deben construir colliders usando la definición estricta y real (ej. `ShapeType.Box` con `BoxShape`, o `ShapeType.Circle` con `CircleShape`), no discriminadores string (`"box"` / `"circle"`).
...
* Modificar `packages/core/src/physics/collision/CollisionSystems.ts` (específicamente `CollisionSystem2D`) para exponer un *flag* de configuración que permita alternar en caliente entre `BroadPhase.sweepAndPrune` y `SpatialHashGrid.findCandidatePairs` sin modificar el narrowphase.
* Ejecutar una regresión completa en los 5 juegos del monorepo (Asteroids, Space Invaders, Pong, Flappy Bird, Geometry Wars) usando el nuevo grid.
