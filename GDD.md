...
| `PLAYER_SHOOT_COOLDOWN`        | `300` ms   | `100`     | `1000`    | Minimum firing interval. `GAME_CONFIG.PLAYER_SHOOT_COOLDOWN`                                        |
...
- [ ] **The Session Hook (Retention Gate)**
  ...
  - Render the **Passport Overlay** displaying XP progress. Explicitly state the progress made toward unlocking their first beneficial mutator (e.g., _"You earned 120 XP! Just 180 XP more to unlock Combo Head Start!"_).
...
- **Output Mutation**:
  - When the game initializes, reduce over active mutators list:
    ```typescript
    const config = activeMutators.reduce(
      (cfg, mutator) => mutator.apply(cfg),
      baseConfig
    );
    ```
  - Inside the individual mutator's configuration apply function (defined in `MutatorConfig.ts`):
    ...
    apply: (cfg) => ({
      ...cfg,
      PLAYER_BULLET_SPEED: ((cfg.PLAYER_BULLET_SPEED as number) || 500) * 1.1,
      BULLET_SPEED: ((cfg.BULLET_SPEED as number) || 300) * 1.1,
    });
    ```
...
_Nota_: Las campañas o nodos simples (como `MultiGameTestCampaign.ts`) pueden seguir utilizando la transición ligera basada únicamente en `sceneToLoad` y objetivos directos cuando no requieran evaluación declarativa de modificadores o reglas de resultado complejas.
