...
> {
  ...
  public updateFromServer(state: Record<string, unknown>, localSessionId?: string) {
    ...
    // TODO(refactor): código duplicado detectado (bloque) con flappybird/FlappyBirdGame.ts:481-493. Considerar extraer a función compartida. Ref: c3ed8066
    ...
    if (state.enemies) {
        Object.entries(state.enemies as Record<string, any>).forEach(([id, p]) => {
            ...
            // TODO(refactor): código duplicado detectado (bloque) con space-invaders/SpaceInvadersGame.ts:828-834. Considerar extraer a función compartida. Ref: 2738d330
            if (entityId !== undefined) entries.push({ entityId, x: p.x, y: p.y, rotation: p.angle });
        });
    }
    if (state.bullets) {
        Object.entries(state.bullets as Record<string, any>).forEach(([id, p]) => {
            ...
            // TODO(refactor): código duplicado detectado (bloque) con flappybird/FlappyBirdGame.ts:499-513. Considerar extraer a función compartida. Ref: 5b8ab361
            if (entityId !== undefined) entries.push({ entityId, x: p.x, y: p.y, rotation: p.angle });
        });
    }
