...
export class PowerUpSystem extends System<CoreComponentRegistry & { PowerUp: PowerUpComponent }> {
  public update(world: World<CoreComponentRegistry & { PowerUp: PowerUpComponent }>, _deltaTime: number): void {
    ...
    const registryInstance = world.getResource<PowerUpEffectRegistry>("PowerUpEffectRegistry");
    const recordRegistry = world.getResource<Record<string, IPowerUpEffect>>("PowerUpEffects");
    ...
    for (const entity of entities) {
      ...
      for (const col of collisionsComp.collisions) {
        ...
        if (isPlayer) {
          const effect = registryInstance?.get(powerUp.powerUpType) ?? recordRegistry?.[powerUp.powerUpType];
