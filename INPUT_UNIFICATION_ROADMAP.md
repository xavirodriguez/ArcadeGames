...
* **Space Invaders:** Traducir `axes.moveX` a su estado interno (`moveLeft`/`moveRight`) y `actions.has("fire")` a `shoot`.
* **Asteroids:** Mapear `axes.moveX` a `rotateLeft`/`rotateRight`, `axes.moveY` (negativo/positivo) a `thrust`, y acciones a `hyperspace`/`fire`.
...
* Implementar un test en el entorno de CI que falle inmediatamente si los archivos `NetTypes.ts` (específicamente la interfaz `InputFrame`) de `packages/network` y de `server/src` divergen en propiedades o tipos.
